...
test.describe('Form layout page', () => {
    ...
    test('input fields', async({page}, testInfo) => {
        ...
        expect(inputValue).toEqual('test1@gmail.com')

        //locator assertion
        await expect(UsingTheGridEmailInput).toHaveValue('test1@gmail.com')
    })
...
test('Dropdown', async({page}) => {
    const drop = page.locator('ngx-header nb-select')
     await drop.click()
...
test('Dialog box', async({page}) => {
    ...
    await expect(page.locator('tbody tr').first().locator('td').nth(5)).not.toHaveText('mdo@gmail.com')
...
test('Web table', async({page}) => {
    ...
    // 1. get by row by any test in this row
    ...
    await page.locator('input-editor').getByPlaceholder('First Name').fill('Dark Magic')
    await page.locator('.nb-checkmark').click()
    await expect(targetRowById.locator('td').nth(2)).toHaveText('Dark Magic')
